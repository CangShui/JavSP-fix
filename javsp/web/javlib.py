...
def _is_cf_challenge(resp_text: str) -> bool:
    """检查返回页面是否为Cloudflare挑战页"""
    text_lower = resp_text.lower()
    return ('just a moment' in text_lower) or ('cf-challenge' in text_lower)
...
def parse_data(movie: MovieInfo):
    ...
    if resp.status_code in (403, 429):
        raise SiteBlocked(f"JavLib: {resp.status_code} 禁止访问，可能触发反爬或地区限制")
    if resp.status_code >= 400:
        raise WebsiteError(f"JavLib: {resp.status_code} 非预期状态码: {url}")
    if _is_cf_challenge(resp.text):
        raise SiteBlocked('JavLib: 检测到Cloudflare挑战页，当前网络环境无法直接抓取')
    html = resp2html(resp)
    if resp.history:
    ...
    container_tags = html.xpath("/html/body/div/div[@id='rightcolumn']")
    if not container_tags:
        raise WebsiteError(f"JavLib: 页面结构已变化或受限: {new_url}")
    container = container_tags[0]
    title_tag = container.xpath("div/h3/a/text()")
    cover_tag = container.xpath("//img[@id='video_jacket_img']/@src")
    info_tags = container.xpath("//div[@id='video_info']")
    if not (title_tag and cover_tag and info_tags):
        raise WebsiteError(f"JavLib: 页面结构不完整，无法解析: {new_url}")
    title = title_tag[0]
    cover = cover_tag[0]
    info = info_tags[0]
    dvdid_tag = info.xpath("div[@id='video_id']//td[@class='text']/text()")
    publish_date_tag = info.xpath("div[@id='video_date']//td[@class='text']/text()")
    duration_tag = info.xpath("div[@id='video_length']//span[@class='text']/text()")
    if not (dvdid_tag and publish_date_tag and duration_tag):
        raise WebsiteError(f"JavLib: 页面缺少关键字段，无法解析: {new_url}")
    dvdid = dvdid_tag[0]
    publish_date = publish_date_tag[0]
    duration = duration_tag[0]
    ...
    producer = producer_tag[0] if producer_tag else None
