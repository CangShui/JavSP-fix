...
def parallel_crawler(movie: Movie, tqdm_bar=None):
    ...
    def wrapper(parser, info: MovieInfo, retry):
        ...
        for cnt in range(retry):
            try:
                ...
                # 非网络类业务错误通常重试无意义，直接结束该抓取器任务
                logger.error(e)
                break
            except requests.exceptions.RequestException as e:
                status_code = getattr(getattr(e, 'response', None), 'status_code', None)
                if status_code is not None and status_code in (400, 401, 403, 404, 405, 410, 451):
                    logger.debug(f'{crawler_name}: HTTP {status_code}，停止重试: {repr(e)}')
                    break
    ...
    for info in all_info.values():
        del info.success
    # 兼容旧版本键名（如'web.javdb'）并保持当前键名（如'javdb'）不被误截断
    normalized_info = {}
    for k, v in all_info.items():
        if k.startswith('web.'):
            normalized_info[k[4:]] = v
        else:
            normalized_info[k] = v
    all_info = normalized_info
    return all_info
            ...
            if Cfg().summarizer.extra_fanarts.enabled:
                ...
                if movie.info.preview_pics:
                    ...
                    for (id, pic_url) in enumerate(movie.info.preview_pics):
                        ...
                        fanart_destination = get_pic_path(f"{extrafanartdir}/{id}.jpg", pic_url)
                        try:
                            ...
                            if valid_pic(fanart_destination):
                                ...
                                saved_name = os.path.basename(fanart_destination)
                                logger.info(f"已下载剧照{pic_url} {saved_name}: {width}x{height}, {filesize} [{elapsed}, {speed}]")
                            else:
                                logger.warning(f"下载剧照失败(无效图片)，已跳过: {pic_url}")
                                if os.path.exists(fanart_destination):
                                    os.remove(fanart_destination)
                        except Exception as e:
                            logger.warning(f"下载剧照失败，已跳过: {pic_url}")
